...
One-a-a-time SNP effect estimates from an analysis of `n=15,000` people for 20 SNPs, divided into two regions (`snps.region1` and `snps.region2`) are stored in `marginal.betas`. The true effects used in the simulations are stored in `true.betas`. You can see the level of confounding due to LD by comparing `marginal.betas` vs `true.betas`. For region 1 a single effect at SNP 5 was simulated, and for region 2 two effects were simulated. 
...
# Application to binary data (log-odds ratios)
JAM is formulated as a linear model (with Gaussian residuals), and this formulation is responsible for the efficiency due to the conjugate results available for this class of models. In order to apply JAM to binary data (i.e. marginal Odds Ratios from logistic regressions) it is necessary to invoke a transformation between the logistic and linear scales. Fortunately, an approximate transformation has been derived by Pirinen et al [@pirinen2013]. 

## Simpler linear transformation
Pirinen et al derived a linear approximation of $\beta$, the effect estimates we would obtain analysing binary data with a linear regression model, in terms of $\gamma$, the reported summary log-odds ratios:

$$
\hat{\beta} = \hat{\gamma} \phi (1-\phi)
$$

where $\phi$ is the proportion of cases in the sample.

## Prior for the residual variance
After using the above formula to transform the log-odds ratios to the linear scale used by JAM, we must also consider an appropiate prior for the residual variance parameter. Under JAM, the prior distribution for $\sigma^2$ is inverse-gamma:
$$
...
$$
$$
Var(\sigma^2) = \frac{b^2}{ (a-1)^2 (a-2)}
$$
Note that $a>2$ to insure a non-negative variance. The residual variance for a binary outcome under the null linear model would be equvialent to the bernoulli variance, which is given by:
$$
\hat{\sigma^2} = \phi (1 - \phi)
$$
Since genetic effects are generally small, when applying JAM with binary data we choose an informative prior centred on this bernoulli variance which would be the correct value if the binary data were analysed with JAM under a null model. To insure an informative prior we start with $a=n$. In order to achieve the desired expectation $b$ is then given by:
$$
b = (n-1) \phi (1-\phi)
$$

## Example
The example dataset also includes marginal log-Odds Ratios from analyses of binary data simulated
from exactly the same genotype matrix as above for $n=15,000$, with causal effects placed at the same two SNPs. To analyse these data with JAM and flag them as log-Odds ratios, we must also pass
the n.cases argument (included in the example data). This tells JAM to invoke the approximate logistic -> linear transform above, and the corresponding prior for the residual variance.
...
JAM has correctly placed compelling evidence at both causal SNPs:

```{r}
PrettyResultsTable(binary.results)
```


# Additions to JAM:
Simply add a n.case and n.control options? Which will then invoke the transformation and set the inverse-gamma prior?
